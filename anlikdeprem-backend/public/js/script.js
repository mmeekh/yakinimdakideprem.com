...
function updateMap() {
  ...
  AppState.earthquakeData.forEach((eq) => {
    ...
    // Daha belirgin renk farkları
    let color, fillColor;
    if (eq.magnitude >= 6.0) {
      // 6.0+ büyük depremler - Koyu kırmızı
      color = '#8B0000';
      fillColor = '#DC143C';
    } else if (eq.magnitude >= 5.0) {
      // 5.0-5.9 orta büyük depremler - Kırmızı
      color = '#DC143C';
      fillColor = '#FF4500';
    } else if (eq.magnitude >= 4.0) {
      // 4.0-4.9 orta depremler - Turuncu-kırmızı
      color = '#FF4500';
      fillColor = '#FF6347';
    } else if (eq.magnitude >= 3.0) {
      // 3.0-3.9 küçük depremler - Turuncu
      color = '#FF6347';
      fillColor = '#FFA500';
    } else {
      // 3.0 altı çok küçük depremler - Sarı-turuncu
      color = '#FFA500';
      fillColor = '#FFFF00';
    }
