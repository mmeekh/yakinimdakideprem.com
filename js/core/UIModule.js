...
class UIModule {
    ...
    return `
      <div class="earthquake-item" data-id="${earthquake.id}">
        ...
        <div class="earthquake-item__details">
          ...
          <p class="earthquake-item__time">${earthquake.time.toLocaleString('tr-TR')}</p>
  ...
  getMagnitudeColor(magnitude) {
    if (magnitude >= 6) return '#d32f2f'; // Dark red
    if (magnitude >= 5) return '#f44336'; // Red
    if (magnitude >= 4) return '#ff9800'; // Orange
    if (magnitude >= 3) return '#ffc107'; // Yellow
    return '#4caf50'; // Green
  }
